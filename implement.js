...
var classFlags = constants.ACC_PUBLIC | constants.ACC_FINAL | constants.ACC_SYNTHETIC,
    methodFlags = constants.ACC_PUBLIC,
...
exports.build = function (name, interfaces, methods) {
    ...
    builder.defineField('$', 'J', constants.ACC_PUBLIC); // pointer to the c++ handle
    ...
    for (var ii = 0, ll = methods.length; ii < ll; ii++) {
        ...
        if (callType in instructions) {
            ...
            code = callInstructions(nameIdx, pointerIdx, argTypes, retType, callerIdx);
        ...
        builder.defineMethod(methodName, methodTypes, constants.ACC_PUBLIC, [builder.createCode(argTypes.length + 3, argTypes.length + 1, code)]);
    }
...
function callInstructions(nameIdx, pointerIdx, argTypes, retType, callerIdx) {
